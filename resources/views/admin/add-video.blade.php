...
@section('h1', 'Редактировать видео')
@section('href', route('admin.edit.modules', ['id' => $module->course->id]))
@section('content')
      <form class="form-lecture" id="formLecture" action="{{route('admin.edit.module.videos.handler', ['id' => $module->id])}}" method="POST"  enctype="multipart/form-data">
        @csrf
        <div class="container">
          <div class="row g-3 all-input-lecture">
          @if(!empty($errors->first('video.*')))
          <div class="invalid-feedback d-block">{{$errors->first('video.*')}}
          </div>            
          @endif
          @if(!empty($errors->first('video')))
          <div class="invalid-feedback d-block">{{$errors->first('video')}}
          ...
          <div class="col-xl-4 col-lg-6 ">
            <div class="lecture-create">       
              <input type="file" name="video[]" class="form-control-file input-lecture">
            </div>
          </div>
          </div>
        </div>
      ...
      @if($module->list)
        <div class="row g-3 all-input-lecture" style="margin-top: 20px;">
          ...
          <div class="col-xl-4 col-lg-6">
            <div class="lecture-create">       
              <div class="downloaded-file"><form action="{{route('admin.delete.video.handler', ['id' => $module->id, 'key' => $key])}}" method="POST">@csrf<button class="del-module del-but">✖</button></form><a href="/{{$el}}" class="documents-to-fill-out-link">Файл видео ({{$key}})</a></div>
            </div>
          </div>
          @endforeach
        </div>
